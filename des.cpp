...
public:
    ...
    string process(string input, bool decrypt = false) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            string sbox_out = "";
            for (int j = 0; j < 8; j++) {
                string block = xored.substr(j * 6, 6);
            ...
            string perm2 = "";
            for (int j = 0; j < 32; j++)
                perm2 += sbox_out[permutation_tab[j] - 1];
...
int main() {
    ...
    if (mode == 1 || mode == 2) {
        ...
        for (int i = 0; i < input.size(); i += 64) {
            ...
            result += des.process(block, mode == 2);
        }
    }
        ...
        DES des1(KeyGenerator(key).generate());
        DES des2(KeyGenerator(k2).generate());
        DES des3(KeyGenerator(k3).generate());
        ...
        for (int i = 0; i < input.size(); i += 64) {
            ...
            if (mode == 3) {
                block = des1.process(block, false);
                block = des2.process(block, true);
                block = des3.process(block, false);
            } else {
                block = des3.process(block, true);
                block = des2.process(block, false);
                block = des1.process(block, true);
            }
